...
The card adjusts date/time labels automatically so they stay readable (especially on phones). With **two or more windows**, the shared axis length follows the **largest nominal slot count** at the chart’s aggregation (e.g. February vs March at daily grain uses 31 slots); **forecast progress** still uses **window 0** only, so the axis can be longer than the forecast period.
