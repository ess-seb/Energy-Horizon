...
The card adjusts date/time labels automatically so they stay readable (especially on phones). With **two or more windows**, the shared axis length follows the **largest nominal slot count** at the chart’s aggregation (e.g. February vs March at daily grain uses 31 slots); tick dates follow **window 0’s** calendar (with an ordinal tail if the axis is longer), and the **“now”** marker is based on today **inside window 0**. **Forecast progress** still uses **window 0** only, so the axis can be longer than the forecast period. **Tooltip** headers use that same slot index and the **Home Assistant time zone** for calendar text (not the browser’s zone). For YoY/MoY/MoM, adaptive labels drop redundant years or months so the axis matches the comparison story (see wiki mental model).
