...
**Analyze follow-up**: Uzupełnienia po `/speckit.analyze` — **FR-H** (T007), rozbicie propagacji stanu vs prognoza (T008–T009), **FR-C N≥3** (T016), **a11y** (T026).
...
- [ ] T001 Create golden scenario matrix (presets + representative overrides, FR-H) in `specs/006-time-windows-unify/golden-scenarios.md`
...
- [ ] T004 Unify `buildChartTimeline` in `src/card/ha-api.ts` for **FR-B** (N=2) and **Longest-window axis span / FR-C** (N>2) per `specs/006-time-windows-unify/contracts/unified-time-windows-axis.md`; keep `forecastPeriodBuckets` from window 0 (**FR-D**)
...
**Goal**: Podsumowanie, oś X, tooltip i prognoza opisują tę samą narrację czasu dla okien efektywnych (**FR-B**, **FR-H**).

**Independent Test**: Dwa okna porównawcze — teksty okresów, znaczenie ticków osi i tooltip zgodne z oknem bieżącym; prognoza i liczby w podsumowaniu zgodne z `forecastPeriodBuckets` / timeline z `src/card/ha-api.ts`.
...
- [ ] T009 [US1] Ensure `computeForecast`, summary / `TextSummary` (or equivalent text stats), and cumulative LTS→series mapping in `src/card/cumulative-comparison-chart.ts` consume the **same** `forecastPeriodBuckets` and timeline semantics exported from `src/card/ha-api.ts` — brak rozjazdu „oś vs prognoza vs podsumowanie”
- [ ] T010 [P] [US1] Align ECharts axis category/time labels with current-window grain in `src/card/echarts-renderer.ts` (**FR-B**, **FR-H**)
- [ ] T011 [P] [US1] Align compact period captions with effective windows in `src/card/labels/compact-period-caption.ts`
- [ ] T012 [US1] Extend expectations for `buildChartTimeline` (two-window **FR-B**) in `tests/unit/ha-api.test.ts`
...
**Goal**: Parzystość YoY / MoY / MoM i typowych nadpisań `time_window` (**SC-1**); pokrycie **FR-C** dla **N≥3**.
...
- [ ] T016 [P] [US2] Add Vitest in `tests/unit/ha-api.test.ts` for **≥3** `ResolvedWindow[]`: `timeline.length` equals **max** per-window nominal slot count at `windows[0].aggregation` (**Longest-window axis span / FR-C**); assert `forecastPeriodBuckets` still from window 0 (**FR-D**)
...
- T005, T006, T007 równolegle po T003–T004 (uwaga: konflikty merge w PR)  
- T010, T011 równolegle po T008–T009  
- T014, T016, T021, T022, T023, T024, T026 równolegle w osobnych plikach  
...
1. Phase 4 (US2) — złote presety + overrides + **T016** (FR-C)  
2. Phase 5 (US3) — carry-forward  
3. Phase 6 (US4) — walidacja  
4. Phase 7 — docs + changelog + **T026** a11y  
...
- **SC-4** (zewnętrzny recenzent wiki) — weryfikacja procesowa po T024–T025, bez osobnego task ID.
