...
### Session 2026-04-20 (chart grid bottom — constant reserve & reduced gap)

- **Chart height MUST be stable** across all X-axis modes (adaptive rich / standard) and all “today” positions. The `grid.bottom` reservation MUST be a **fixed constant** derived from the full two-line typography budget (`AXIS_TICK_LABEL_GAP_PX + edge lineHeight + today lineHeight + descender buffer`), regardless of whether “today” is currently in range or whether adaptive rich labels are active. Dynamic switching between `0` and the two-line value causes ~20 px height drift.
- **`containLabel: false` is required.** With `containLabel: true`, `grid.bottom: N` is an outer margin added *below* the auto-managed label region—not the total label budget—which produces redundant space and makes `grid.bottom` semantically incorrect for explicit px calculations. `containLabel: false` means `grid.bottom` is the exact distance from the chart canvas edge to the axis line; labels fill that space downward.
- **`AXIS_TICK_LABEL_GAP_PX`** (gap between axis tick mark and the top of the label block) is the primary lever for adjusting the visible bottom margin; reducing it from 8 → 4 px tightens the bottom without touching typography.
- **`GRID_LEFT_PX`** replaces `containLabel: true` auto-expansion for Y-axis labels; it is an explicit px constant sized for the widest realistic formatted Y label (e.g. “1,927 kWh”). If future Y labels exceed this, increase the constant.
- **`CHART_MIN_HEIGHT_BASE_PX`** is adjusted upward (274 px from 240 px) to keep the plot area comparable to the old adaptive case now that the dynamic `xAxisLabelMinHeightExtraPx` contribution to container min-height is removed. No new YAML keys.
